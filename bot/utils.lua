...
local function is_superuser(user_id)
  local var = false
  local users = user_id
  for v,user in pairs(_config.super_users) do
    if user == users then
      var = true
    end
  end
...
-- Check if user_id is globally muted or not
...
function mute_list(chat_id)
  local hash =  'muted:'..chat_id
  local list = redis:smembers(hash)
  local text = "muted users list !\n\n"
  for k,v in pairs(list) do
 		local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
   	text = text..k.." - "..string.gsub(user_info.print_name, "_", " ").." ["..v.."]\n"
...
--mute by reply for admins
function mute_by_reply(extra, success, result)
  if result.to.type == 'channel' then
  local chat = 'channel#id'..result.to.id
  if tonumber(result.from.id) == tonumber(our_id) then -- Ignore bot
      return "I won't mute myself"
  end
  if is_momod2(result.from.id, result.to.id) then -- Ignore mods,owner,admin
    return "you can't mute mods,owner and admins"
  end
  mute_user(result.from.id, result.to.id)
  send_large_msg(chat, "User "..result.from.id.." muted")
  else
    return 'Use This in Your Groups'
  end
end
...
-- mute by reply for admins
function mute_by_reply_admins(extra, success, result)
  if result.to.type == 'channel' then
    local chat = 'channel#id'..result.to.id
    if tonumber(result.from.id) == tonumber(our_id) then -- Ignore bot
      return "I won't mute myself"
...
-- unmute by reply
function unmute_by_reply(extra, success, result) 
  if result.to.type == 'channel' then
    local chat = 'channel#id'..result.to.id
    if tonumber(result.from.id) == tonumber(our_id) then -- Ignore bot
      return "I won't unmute myself"
    end
    send_large_msg(chat, "User "..result.from.id.." Unmuted")
    -- Save on redis
    local hash =  'muted:'..result.to.id
    redis:srem(hash, result.from.id)
  else
    return 'Use This in Your Groups'
  end
